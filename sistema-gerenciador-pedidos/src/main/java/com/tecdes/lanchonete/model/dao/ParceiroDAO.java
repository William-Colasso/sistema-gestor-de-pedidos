...
public class ParceiroDAO implements InterfaceDAO<Parceiro> {
    ...
    @Override
    public void create(Parceiro t) {
        try (Connection conn = ConnectionFactory.getConnection()) {
            String sql;
            PreparedStatement pr;

            sql = "INSERT INTO T_SGP_CLIENTE (nm_parceiro, ds_email, nr_telefone) VALUES (?,?,?)";
    ...
    public void delete(Long id) {
        try (Connection conn = ConnectionFactory.getConnection()) {
            ...
            sql = "DELETE FROM T_SGP_CLIENTE where id_parceiro = ?";
            pr = conn.prepareStatement(sql);
            pr.setLong(1, id);
    ...
    public void update(Parceiro t) {
        try (Connection conn = ConnectionFactory.getConnection()) {
            ...
            sql = "UPDATE T_SGP_CLIENTE SET nm_parceiro = ?, ds_email = ?, nr_telefone = ? where id_parceiro = ?";
            pr = conn.prepareStatement(sql);
            pr.setString(1, t.getNome());
            pr.setString(2, t.getEmail());
            pr.setString(3, t.getTelefone());
